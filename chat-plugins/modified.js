...
exports.commands = {
	...
	authority: function (target, room, user, connection) {
		...
		for (let u in Users.usergroups) {
			...
			if (ranks.includes(rank)) {
				...
				if (name) rankLists[rank].push(Tsunami.nameColor(name, (Users(name) && Users(name).connected)));
			}
		}
	...
	roomauth: function (target, room, user, connection, cmd) {
		...
		let buffer = Object.keys(rankLists).sort((a, b) =>
			...
			roomRankList = roomRankList.map(s => ((Users(s) && Users(s).connected) ? Tsunami.nameColor(s, true) : Tsunami.nameColor(s)));
		...
		if (targetRoom.founder) {
			buffer.unshift((targetRoom.founder ? "Room Founder:\n" + ((Users(targetRoom.founder) && Users(targetRoom.founder).connected) ? Tsunami.nameColor(targetRoom.founder, true) : Tsunami.nameColor(targetRoom.founder)) : ''));
		}
	...
	roomfounder: function (target, room, user) {
		...
		if (targetUser) {
			targetUser.popup(`|html|You were appointed Room Founder by ${Tsunami.nameColor(user.name, true)} in ${room.title}.`);
	...
	roomowner: function (target, room, user) {
		...
		if (targetUser) {
			targetUser.popup(`|html|You were appointed Room Owner by ${Tsunami.nameColor(user.name, true)} in ${room.title}.`);
			room.onUpdateIdentity(targetUser);
		}
		Rooms.global.writeChatRoomData();
	},
	...
	roompromote: function (target, room, user, connection, cmd) {
		...
		if (this.pmTarget && targetUser) {
			...
			if (needsPopup) targetUser.popup(`|html|You were demoted to Room ${groupName} by ${Tsunami.nameColor(user.name, true)} in ${room.title}.`);
		} else if (nextGroup === '#') {
			this.addModCommand(`${'' + name} was promoted to ${groupName} by ${user.name}.`);
			if (needsPopup) targetUser.popup(`|html|You were promoted to ${groupName} by ${Tsunami.nameColor(user.name, true)} in ${room.title}.`);
		} else {
			this.addModCommand(`${'' + name} was promoted to Room ${groupName} by ${user.name}.`);
			if (needsPopup) targetUser.popup(`|html|You were promoted to Room ${groupName} by ${Tsunami.nameColor(user.name, true)} in ${room.title}.`);
		}
	...
	away: function (target, room, user, connection, cmd) {
		...
		if (user.can('mute', null, room)) this.add("|raw|-- " + Tsunami.nameColor(user.name, true) + " is now " + target.toLowerCase() + ".");
	...
	back: function (target, room, user) {
		...
		if (statusIdx < 0) {
			user.isAway = false;
			if (user.can('mute', null, room)) this.add("|raw|-- " + Tsunami.nameColor(user.userid, true) + " is no longer away.");
			return false;
		}
		...
		user.updateIdentity();
		user.isAway = false;
		if (user.can('mute', null, room)) this.add("|raw|-- " + Tsunami.nameColor(user.userid, true) + " is no longer " + status.toLowerCase() + ".");
