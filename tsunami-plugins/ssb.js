...
function buildMenu(userid) {
	...
	let split = Tsunami.ssb[userid].species.split('-');
	if (split.length > 1) {
		speciesName = toId(split[0]) + '-' + speciesName.substring(toId(split[0]).length);
	}
	let output = '';
	output += '<div class="setchart" style="height: 155px; background-image:url(//play.pokemonshowdown.com/sprites/' + (Dex.getTemplate(toId(Tsunami.ssb[userid].species)).gen === 7 ? 'bw' : 'xydex') + '' + (Tsunami.ssb[userid].shiny ? '-shiny' : '') + '/' + speciesName + '.png); background-position: -2px -3px; background-repeat: no-repeat;">';
	output += '<div class="setcol setcol-icon"><div class="setcell-sprite"></div><div class="setcell setcell-pokemon"><label>Pokémon</label><button class="textbox chartinput" style="width:104px; height: 20px; text-align: left" name="send" value="/ssb edit species">' + Tsunami.ssb[userid].species + '</button></div></div>';
	output += '<div class="setcol setcol-details"><div class="setrow"><div class="setcell setcell-details"><label>Details</label><button class="textbox setdetails" tabindex="-1" name="send" value="/ssb edit details"><span class="detailcell detailcell-first"><label>Level</label>' + Tsunami.ssb[userid].level + '</span><span class="detailcell"><label>Gender</label>' + (Tsunami.ssb[userid].gender === 'random' ? '-' : Tsunami.ssb[userid].gender) + '</span><span class="detailcell"><label>Happiness</label>' + Tsunami.ssb[userid].happiness + '</span><span class="detailcell"><label>Shiny</label>' + (Tsunami.ssb[userid].shiny ? 'Yes' : 'No') + '</span></button><span class="itemicon" style="background: none"></span></div></div><div class="setrow"><div class="setcell setcell-item"><label>Item</label><button class="textbox chartinput" style="width:104px; height: 20px; text-align: left" name="send" value="/ssb edit item">' + (Tsunami.ssb[userid].item ? Tsunami.ssb[userid].item : '') + '</button></div><div class="setcell setcell-ability"><label>Ability</label><button class="textbox chartinput" style="width:104px; height: 20px; text-align: left" name="send" value="/ssb edit ability">' + Tsunami.ssb[userid].ability + '</button></div></div></div>';
	output += '<div class="setcol setcol-moves"><div class="setcell"><label>Moves</label><button class="textbox chartinput" style="width:129px; height: 20px; text-align: left; overflow: hidden" name="send" value="/ssb edit move">' + (Tsunami.ssb[userid].movepool[0] ? Tsunami.ssb[userid].movepool[0] : '') + '</button></div><div class="setcell"><button class="textbox chartinput" style="width:129px; height: 20px; text-align: left; overflow: hidden" name="send" value="/ssb edit move">' + (Tsunami.ssb[userid].movepool[1] ? Tsunami.ssb[userid].movepool[1] : '') + '</button></div><div class="setcell"><button class="textbox chartinput" style="width:129px; height: 20px; text-align: left; overflow: hidden" name="send" value="/ssb edit move">' + (Tsunami.ssb[userid].movepool[2] ? Tsunami.ssb[userid].movepool[2] : '') + '</button></div><div class="setcell"><button class="textbox chartinput" style="width:129px; height: 20px; text-align: left; overflow: hidden" name="send" value="/ssb edit move">' + (Tsunami.ssb[userid].cMove ? Tsunami.ssb[userid].cMove : (Tsunami.ssb[userid].movepool[3] ? Tsunami.ssb[userid].movepool[3] : '')) + '</button></div></div>';
	...
	for (let i = 0; i < statNames.length; i++) {
		stats[toId(statNames[i])] = getStat(toId(statNames[i]), {
			species: Tsunami.ssb[userid].species,
			evs: Tsunami.ssb[userid].evs,
			ivs: Tsunami.ssb[userid].ivs,
			nature: Tsunami.ssb[userid].nature,
			level: Tsunami.ssb[userid].level,
		});
		let evBuf = '<em>' + (Tsunami.ssb[userid].evs[toId(statNames[i])] === 0 ? '' : Tsunami.ssb[userid].evs[toId(statNames[i])]) + '</em>';
		if (Dex.getNature(Tsunami.ssb[userid].nature).plus === toId(statNames[i])) {
			evBuf += '<small>+</small>';
		} else if (Dex.getNature(Tsunami.ssb[userid].nature).minus === toId(statNames[i])) {
			evBuf += '<small>&minus;</small>';
		}
		let width = stats[toId(statNames[i])] * 75 / 504;
		if (statNames[i] === 'HP') width = stats[toId(statNames[i])] * 75 / 704;
		if (width > 75) width = 75;
		let color = Math.floor(Tsunami.ssb[userid].evs[toId(statNames[i])] * 180 / 714);
	...
	output += '<div style="text-align:center"><button class="button" name="send" value="/ssb custom">Custom Move List</button> | <button class="button" name="send" value="/ssb toggle">' + (Tsunami.ssb[userid].active ? 'Deactivate your pokemon' : 'Activate your pokemon') + '</button></div></div>';
	return output;
}
...
function moveMenu(userid) {
	...
	for (let i = 0; i < Tsunami.ssb[userid].movepool.length; i++) {
		if (Tsunami.ssb[userid].movepool.length === 0) break;
		output += ((i + 1 === Tsunami.ssb[userid].movepool.length && !Tsunami.ssb[userid].cMove) ? Tsunami.ssb[userid].movepool[i] : Tsunami.ssb[userid].movepool[i] + ', ');
	}
	if (Tsunami.ssb[userid].cMove) output += Tsunami.ssb[userid].cMove;
	output += '</div><div style="padding-bottom: 2px"><i>Custom-made Custom Move:</i> ' + (Tsunami.ssb[userid].selfCustomMove ? Tsunami.ssb[userid].selfCustomMove : '<button name="send" value="/shop" class="button">Purchase</button>') + '</div>';
	output += '<button name="send" class="button" value="/ssb edit move help">Set Moves</button> | <button class="button" name="send" value="/ssb custom">Set a Custom Move</button> | <button name="send" class="button" value="/ssb edit moveq custom, ' + (Tsunami.ssb[userid].selfCustomMove ? Tsunami.ssb[userid].selfCustomMove : '') + '">Set Custom-made Custom Move</button> | <button name="send" class="button" value="/ssb edit main">Main Menu</button></div>';
	return output;
}

function itemMenu(userid) {
	return '<div class="setchart" style="text-align:center"><h3><u>Item Menu</u></h3><div style="padding-bottom: 2px"><i>Current Item:</i> ' + (Tsunami.ssb[userid].item ? Tsunami.ssb[userid].item : 'None') + '</div><div style="padding-bottom: 2px"><i>Custom Item:</i> ' + (Tsunami.ssb[userid].cItem ? Tsunami.ssb[userid].cItem : '<button name="send" value="/shop" class="button">Purchase</button>') + '</div><button name="send" class="button" value="/ssb edit item help">Set Item</button> | <button name="send" class="button" value="/ssb edit itemq reset">Reset Item</button> | <button name="send" class="button" value="/ssb edit itemq ' + (Tsunami.ssb[userid].cItem ? Tsunami.ssb[userid].cItem : 'help') + '">Set Custom Item</button> | <button name="send" class="button" value="/ssb edit main">Main Menu</button></div>';
}

function abilityMenu(userid) {
	let output = '<div class="setchart" style="text-align:center"><h3><u>Ability Menu</u></h3><div style="padding-bottom: 2px"><i>Current Ability:</i> ' + Tsunami.ssb[userid].ability + '</div><div style="padding-bottom: 2px"><i>Custom Ability:</i> ' + (Tsunami.ssb[userid].cAbility ? Tsunami.ssb[userid].cAbility : '<button name="send" value="/shop" class="button">Purchase</button>') + '</div>';
	let pokemon = Dex.getTemplate(Tsunami.ssb[userid].species);
	...
	if (Tsunami.ssb[userid].cAbility) output += '<button name="send" value="/ssb edit abilityq ' + Tsunami.ssb[userid].cAbility + '" class="button">Set to ' + Tsunami.ssb[userid].cAbility + '</button> | ';
...
function detailMenu(userid) {
	...
	output += '<i>Level: </i>' + Tsunami.ssb[userid].level + ' | <button name="send" value="/ssb edit detailsq level, 1" class="button">Set to 1</button> <button name="send" value="/ssb edit detailsq level, 50" class="button">Set to 50</button> <button class="button" name="send" value="/ssb edit detailsq level, 100">Set to 100</button><br/>';
	output += '<i>Gender: </i>' + Tsunami.ssb[userid].gender + ' | <button name="send" value="/ssb edit detailsq gender, male" class="button">Set to Male</button> <button name="send" value="/ssb edit detailsq gender, female" class="button">Set to Female</button> <button class="button" name="send" value="/ssb edit detailsq gender, random">Set to Random</button> <button name="send" value="/ssb edit detailsq gender, genderless" class="button">Set to Genderless</button><br/>';
	output += '<i>Happiness: </i>' + Tsunami.ssb[userid].happiness + ' | <button name="send" value="/ssb edit details happiness, 0" class="button">Set to 0</button> <button class="button" name="send" value="/ssb edit details happiness, 255">Set to 255</button> <button name="send" value="/ssb edit details happiness" class="button">Set to custom value</button><br/>';
	output += '<i>Shiny?:</i> | ' + (Tsunami.ssb[userid].canShiny ? '<button name="send" value="/ssb edit details shiny" class="button">Toggle Shiny</button>' : '<button name="send" value="/shop" class="button">Purchase</button>') + ' | <i>Custom Symbol: </i>' + (Tsunami.ssb[userid].cSymbol ? ('' + Tsunami.ssb[userid].symbol + ' <button class="button" name="send" value="/ssb edit details symbol">Change</button>') : '<button class="button" name="send value="/shop">Purchase</button>') + ' | <button class="button" name="send" value="/ssb edit main">Main Menu</button></div>';
	return output;
}
...
try {
	if (!noRead) {
		...
		Tsunami.ssb = global.ssb = {};
		//parse JSON back into the SSB class.
		for (let key in raw) {
			Tsunami.ssb[key] = new SSB(raw[key].userid, raw[key].name);
			for (let key2 in Tsunami.ssb[key]) {
				Tsunami.ssb[key][key2] = raw[key][key2];
			}
		}
	} else {
		Tsunami.ssb = global.ssb = {};
	...
	ssb: {
		edit: {
			...
			'': function (target, room, user, connection, cmd, message) {
				...
				let targetUser = Tsunami.ssb[user.userid];
		...
		log: function (target, room, user, connection, cmd, message) {
			...
			case 'view':
				...
				if (toId(target[1]) === 'all') {
					...
					for (let i in Tsunami.ssb) {
						for (let j in Tsunami.ssb[i].bought) {
							let buttons = '<button class="button" name="send" value="/ssb log mark, ' + Tsunami.ssb[i].userid + ', ' + j + ', complete">Mark as Complete</button><button class="button" name="send" value="/ssb log mark, ' + Tsunami.ssb[i].userid + ', ' + j + ', pending">Mark as Pending</button><button class="button" name="send" value="/ssb log mark, ' + Tsunami.ssb[i].userid + ', ' + j + ', remove"><span style="color:red">Remove this purchase</span</button>';
							output += '<tr><td style="border: 1px solid black">' + Tsunami.ssb[i].name + '</td><td style="border: 1px solid black">' + j + '</td><td style="border: 1px solid black">' + (Tsunami.ssb[i].bought[j] ? (Tsunami.ssb[i].bought[j] === 'complete' ? 'Complete' : 'Pending') : 'Removed') + '</td><td style="border: 1px solid black">' + buttons + '</td></tr>';
						}
					}
				} else {
					target[1] = toId(target[1]);
					if (!Tsunami.ssb[target[1]]) return this.errorReply(target[1] + ' does not have a SSBFFA pokemon yet.');
					if (user.can('roomowner')) {
						output += '<th style="border: 1px solid black">Options</th><tr/>';
						for (let j in Tsunami.ssb[target[1]].bought) {
							let buttons = '<button class="button" name="send" value="/ssb log mark, ' + Tsunami.ssb[target[1]].userid + ', ' + j + ', complete">Mark as Complete</button><button class="button" name="send" value="/ssb log mark, ' + Tsunami.ssb[target[1]].userid + ', ' + j + ', pending">Mark as Pending</button><button class="button" name="send" value="/ssb log mark, ' + Tsunami.ssb[target[1]].userid + ', ' + j + ', remove"><span style="color:red">Remove this purchase</span</button>';
							output += '<tr><td style="border: 1px solid black">' + Tsunami.ssb[target[1]].name + '</td><td style="border: 1px solid black">' + j + '</td><td style="border: 1px solid black">' + (Tsunami.ssb[target[1]].bought[j] ? (Tsunami.ssb[target[1]].bought[j] === 'complete' ? 'Complete' : 'Pending') : 'Removed') + '</td><td style="border: 1px solid black">' + buttons + '</td></tr>';
						}
					} else {
						output += '</tr>';
						for (let j in Tsunami.ssb[target[1]].bought) {
							output += '<tr><td style="border: 1px solid black">' + Tsunami.ssb[target[1]].name + '</td><td style="border: 1px solid black">' + j + '</td><td style="border: 1px solid black">' + (Tsunami.ssb[target[1]].bought[j] ? (Tsunami.ssb[target[1]].bought[j] === 'complete' ? 'Complete' : 'Pending') : 'Removed') + '</td></tr>';
						}
					}
				}
			...
			case 'mark':
				...
				if (!Tsunami.ssb[target[1]]) return this.errorReply(target[1] + ' does not have a SSBFFA pokemon yet.');
				if (Tsunami.ssb[target[1]].bought[target[2]] === undefined) return this.parse('/help ssb log');
				switch (target[3]) {
				case 'complete':
					if (Tsunami.ssb[target[1]].bought[target[2]] === target[3]) return this.errorReply(target[1] + '\'s ' + target[2] + ' is already ' + target[3] + '.');
					Tsunami.ssb[target[1]].bought[target[2]] = 'complete';
					writeSSB();
					return this.sendReply(target[1] + '\'s ' + target[2] + ' was marked as complete.');
					//break;
				case 'pending':
					if (Tsunami.ssb[target[1]].bought[target[2]] === true) return this.errorReply(target[1] + '\'s ' + target[2] + ' is already ' + target[3] + '.');
					Tsunami.ssb[target[1]].bought[target[2]] = true;
					writeSSB();
					return this.sendReply(target[1] + '\'s ' + target[2] + ' was marked as pending.');
					//break;
				case 'remove':
					if (Tsunami.ssb[target[1]].bought[target[2]] === false) return this.errorReply(target[1] + '\'s ' + target[2] + ' is already removed.');
					...
					Tsunami.ssb[target[1]].bought[target[2]] = false;
					writeSSB();
		...
		validate: function (target, room, user, connection, cmd, message) {
			...
			let targetUser = Tsunami.ssb[toId(target)];
			...
			if (toId(cmd) !== 'validateall') {
				...
				for (let key in Tsunami.ssb) {
					let valid = validate(this, Tsunami.ssb[key], true);
					if (!valid) {
						Tsunami.ssb[key].active = false;
						if (Users(toId(Tsunami.ssb[key].name))) Users(toId(Tsunami.ssb[key].name)).popup('Your SSBFFA pokemon was deactivated because it is invalid.');
						writeSSB();
						this.errorReply(Tsunami.ssb[key].name + '\'s pokemon was invalid. Invalid parts have been reset and this pokemon was deactivated.');
					}
				}
