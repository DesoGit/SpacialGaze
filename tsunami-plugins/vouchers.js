...
class Voucher {
	constructor(userid, voucher, item, expires, id) {
		...
		this.id = (id ? id : Tsunami.vouchers.storageForVocuherIds);
		if (!id) Tsunami.vouchers.storageForVocuherIds++;
	}
	...
	redeem() {
		...
		case 'item':
			...
			case 'customavatar':
				...
				//Tsunami.messageSeniorStaff(Users(this.userid).name + " has purchased a Custom Avatar. Please contact this user to setup their Custom Avatar.");
				break;
			case 'customcolor':
				...
				//Tsunami.messageSeniorStaff(Users(this.userid).name + " has purchased a Custom Color. Please contact this user to setup their Custom Color.");
				break;
			case 'customtitle':
				...
				//Tsunami.messageSeniorStaff(Users(this.userid).name + " has purchased a Custom Title. Please contact this user to setup their Custom Color.");
				break;
			case 'customicon':
				...
				//Tsunami.messageSeniorStaff(Users(this.userid).name + " has purchased a Userlist Icon. Please contact this user to setup their Userlist Icon.");
				break;
			}
			break;
		case 'boostuno':
	...
	if (!noRead) {
		...
		for (let key in raw) {
			if (key === 'storageForVocuherIds') {
				Tsunami.vouchers.storageForVocuherIds = raw[key];
				continue;
			}
			Tsunami.vouchers[key] = [];
			for (let i = 0; i < raw[key].length; i++) {
				...
				Tsunami.vouchers[key].push(reVouch);
			}
		}
		if (!Tsunami.vouchers.storageForVocuherIds) Tsunami.vouchers.storageForVocuherIds = 1;
	} else {
		Tsunami.vouchers = {
			storageForVocuherIds: 1,
		};
...
exports.commands = {
	...
	voucher: {
		give: function (target, room, user, connection, cmd, message) {
			if (Tsunami.storageForVocuherIds === -1) return this.errorReply('An error occured while loading vouchers. They cannot be used at this time.');
			...
			if (!Tsunami.vouchers[targetUser.userid]) Tsunami.vouchers[targetUser.userid] = [];
			...
			if (index === 0 || index === 1) {
				...
				if (index === 0) {
					...
					Tsunami.vouchers[targetUser.userid].push(voucher);
		...
		take: function (target, room, user, connection, cmd, message) {
			...
			let targetUser = Users(toId(target[0]));
			if (!targetUser) return this.errorReply('User ' + target[0] + ' not found.');
			if (!Tsunami.vouchers[targetUser.userid] || Tsunami.vouchers[targetUser.userid].length === 0) return this.errorReply(targetUser.name + ' has no vouchers.');
			if (isNaN(Number(target[1]))) return this.errorReply('Ids must be a number.');
			for (let key in Tsunami.vouchers[targetUser.userid]) {
				if (Tsunami.vouchers[targetUser.userid][key].id === Number(target[1])) {
					delete Tsunami.vouchers[targetUser.userid][key];
					Tsunami.vouchers[targetUser.userid].splice(key, 1);
					writeFile();
		...
		list: function (target, room, user, connection, cmd, message) {
			...
			if (!Tsunami.vouchers[targetUser.userid] || Tsunami.vouchers[targetUser.userid].length === 0) return this.errorReply(targetUser.name + ' has no vouchers.');
			let list = Tsunami.vouchers[targetUser.userid];
		...
		redeem: function (target, room, user, connection, cmd, message) {
			...
			if (!Tsunami.vouchers[user.userid] || Tsunami.vouchers[user.userid].length === 0) return this.errorReply('You have no vouchers.');
			let index = -1;
			for (let i = 0; i < Tsunami.vouchers[user.userid].length; i++) {
				if (Tsunami.vouchers[user.userid][i].id === target) {
					index = i;
					break;
				}
			}
			if (index === -1) return this.errorReply('You do not own voucher id ' + target + '.');
			switch (Tsunami.vouchers[user.userid][index].redeem()) {
			case true:
				//Successfuly redeemed
				delete Tsunami.vouchers[user.userid][index];
				Tsunami.vouchers[user.userid].splice(index, 1);
				writeFile();
				return true;
				//break;
			case false:
			...
			case 'expired':
				...
				//break;
			case 'active':
				return this.sendReply('You already have an active ' + toId(Tsunami.vouchers[user.userid][index].goodFor) + '.');
				//break;
			case 'token':
				return this.sendReply('You already have a token for a ' + toId(Tsunami.vouchers[user.userid][index].item) + '.');
				//break;
			default:
